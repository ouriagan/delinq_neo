...
La variable *faits de délinquance* est significativement corrélée avec deux des cinq grands domaines du NEO PI-R : avec le caractère Agréable (*r* = -.26, *p* = .005) et avec le caractère Consciencieux (*r* = -.31, *p* = .001). Elle est également corrélée avec les variables d’antisocialité (*r* = .38, *p* < .001) et de psychopathie (*r* = .33, *p* < .001).
Elle est également corrélée avec dix facettes du NEO PI-R : E3 (*r* = .29, *p* = .002), A2 (*r* = -.28, *p* = .003), A3 (*r* = -.21, *p* = .03), A4 (*r* = -.21, *p* = .02), A5 (*r* = -.19, *p* = .05),  C1 (*r* = -25, *p* = .007), C2 (*r* = -.19, *p* = .04), C3 (*r* = -.37, *p* < .001), C5 (*r* = -.26, *p* = .007) et C6 (*r* = -.29, *p* = .002).
Puisque *faits de délinquance* est la variable qui nous intéresse le plus dans cette étude, nous avons testé plusieurs modèles de régression linéaire qui permettent de la prédire à partir des facettes du NEO PI-R. Celui qui explique le mieux sa variance est le modèle présenté dans le Tableau 5.
